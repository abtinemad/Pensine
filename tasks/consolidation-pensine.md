...
DEUX AXES DE MATURATION, à ne pas confondre : la VÉRACITÉ (registres : brute → à vérifier → vérifié) répond à « est-ce vrai ? » et concerne les faits ; l'ÉLABORATION (jalons, voir conventions « Maturité des fils ») répond à « est-ce travaillé ? » et concerne les fils de pensée. Un fil peut être parfaitement vrai et pas travaillé, ou l'inverse.
...
3. CYCLE DE VIE ET MATURITÉ DES FILS. Repère les fils clos (objectif atteint, RDV passé, projet livré) → marque-les clos et déplace-les vers une section « ## Archives » (crée-la en bas de la nette si absente, au-dessus du Journal des briefings). Pour chaque fil ACTIF, évalue les 5 jalons d'élaboration définis dans les conventions (« Maturité des fils » : contexte / ressenti-enjeu / demande ou décision / diffraction / direction), marque les jalons atteints au format compact dans la nette (`Élaboration : 1✓ 2✓ 3✓ 4·seul 5·—`), et remonte dans le rapport le jalon EXACT où chaque fil bloque. Signale en particulier les fils « portés seuls » (jamais confrontés à un autre regard humain, jalon 4 manquant) — une question relationnelle ou stratégique ne se tranche pas entièrement du dedans. Repère aussi les fils qui STAGNENT (aucun mouvement depuis longtemps, échéance dépassée en silence) → remonte-les dans le rapport pour décision, ne les tranche pas.
...
5. MOTIFS RÉCURRENTS. En relisant `pensine-brute.md` sur la durée (pas seulement la semaine écoulée), repère les thèmes qui reviennent et les points d'arrêt habituels — un sujet qui resurgit, un type de fil qui n'atteint jamais sa direction, une même tension qui se rejoue. Nomme-les avec PARCIMONIE dans le rapport : observation, jamais verdict ni diagnostic, et seulement si c'est vraiment récurrent. Dans le doute, tais-toi.

6. MÉTA-REVUE DU SYSTÈME. Prends du recul sur la pensine, le briefing et les conventions eux-mêmes : une rubrique du briefing est-elle devenue inutile ? un type de note récurrent mériterait-il sa propre section ou une nouvelle règle de format ? un projet terminé traîne-t-il encore dans les fils actifs ? la structure brute/nette tient-elle ? Pour chaque ajustement de COMPORTEMENT que tu recommandes : ajoute-le dans la section « ## Propositions en attente de ton feu vert » de `pensine-conventions.md` (via Edit, en append, format `- [YYYY-MM-DD | proposé par Claude] <règle + raison>`), ET liste-le dans le rapport. Tu NE modifies PAS toi-même la tâche briefing-matinal ni les fichiers de skill (hors de ta portée), ni « Règles actives ». Tu proposes, l'utilisateur décide et applique.

SORTIE : un rapport de révision court, structuré (Cohérence brute↔nette — Nettoyage appliqué — Fils clos/archivés — Maturité des fils [jalon de blocage ; fils portés seuls] — Fils qui stagnent [décision requise] — Échéances — Motifs récurrents — Méta-revue [propositions déposées]). Distingue nettement CE QUE TU AS APPLIQUÉ de CE QUI REQUIERT SA DÉCISION. Pas de préambule, pas de conclusion bavarde. Termine par UNE ligne : l'arbitrage le plus important de la semaine pour la pensine. Note explicitement les choix que tu as faits en autonomie.
